...
def get_admissions_statistics(admissions_df: pandas.DataFrame):
    admissions_df.loc[
        admissions_df['field_name'] == 'Consult Status Time',
        'field_value'
    ] = "0"
    unique_pairs_df = admissions_df.groupby(
        ['field_name', 'field_value']).size().reset_index(name='counts')
    ...
    sns.barplot(
        x=unique_pairs_df['field_name'] + ' - ' + unique_pairs_df['field_value'],
        y=unique_pairs_df['counts'], palette='viridis')
